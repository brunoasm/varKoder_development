...
conventional_barcode_prec_recall = results_barcodes %>%
  ...
  map(~{
    ...
    bind_rows(
      calculate_precision_recall(.x, taxonomic_level = 'family') %>% mutate(marker = marker_val),
      calculate_precision_recall(.x, taxonomic_level = 'genus') %>% mutate(marker = marker_val),
      calculate_precision_recall(.x, taxonomic_level = 'species') %>% mutate(marker = marker_val)
    )
  }) %>%
  bind_rows() %>%
...
write_csv(perc_recall_all, 'tables_manuscript/precision_recall_Malpighiales.csv')
```
...
diverse_datasets_prec_rec = other_results %>%
  ...
  group_split() %>%
  map(~{
    calculate_precision_recall(.x) %>% mutate(dataset=.x$dataset[1])
  }) %>%
  bind_rows() %>%
  transmute(dataset=paste("Diverse species-level",dataset,sep=", "),
