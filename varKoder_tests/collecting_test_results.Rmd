...
## Extended figure for conventional barcodes
For the other conventional barcode results, we will make a supplementary Figure.

```{r}
modify_plot <- function(plot, title) {
  plot + ggtitle(title) + theme(legend.position = "none")
}

p3 <- patchwork::wrap_plots(modify_plot(p_barcode_family$matK, 'matK') + theme(axis.text.x = element_blank(),
                                                                               axis.title.x = element_blank(),
                                                                              axis.text.y = element_blank(),
                                                                              axis.title.y = element_blank()),
                            modify_plot(p_barcode_family$ndhF, 'ndhF') + theme(axis.text.x = element_blank(),
                                                                               axis.title.x = element_blank(),
                                                                              axis.text.y = element_blank(),
                                                                              axis.title.y = element_blank()),
                            p_barcode_family$`trnL-F` + ggtitle('trnL-F') + theme(axis.text.y = element_blank(),
                                                                              axis.title.y = element_blank(),
                                                                              axis.title.x = element_blank()),
                            ncol = 1) +
  plot_annotation(title = 'Family')

p2 <- patchwork::wrap_plots(modify_plot(p_barcode_genus$matK, 'matK') + theme(axis.text.x = element_blank(),
                                                                               axis.title.x = element_blank(),
                                                                              axis.text.y = element_blank(),
                                                                              axis.title.y = element_blank()),
                            modify_plot(p_barcode_genus$ndhF, 'ndhF') + theme(axis.text.x = element_blank(),
                                                                               axis.title.x = element_blank(),
                                                                              axis.text.y = element_blank(),
                                                                              axis.title.y = element_blank()),
                            modify_plot(p_barcode_genus$`trnL-F`, 'trnL-F') + theme(axis.text.y = element_blank(),
                                                                              axis.title.y = element_blank()),
                            ncol = 1) +
  plot_annotation(title = 'Genus')

p1 <- patchwork::wrap_plots(modify_plot(p_barcode_species$matK, 'matK') + theme(axis.text.x = element_blank(),
                                                                              axis.title.x = element_blank(),
                                                                              axis.title.y = element_blank()),
                            modify_plot(p_barcode_species$ndhF, 'ndhF') + theme(axis.text.x = element_blank(),
                                                                              axis.title.x = element_blank()),
                            modify_plot(p_barcode_species$`trnL-F`, 'trnL-F') + theme(axis.title.y = element_blank(),
                                                                                      axis.title.x = element_blank()),
                            ncol = 1) +
  plot_annotation(title = 'Species')

add_bounding_box <- function(p, title = "") {
  p +
    annotation_custom(
      grob = grid::linesGrob(arrow = arrow(type = "open", ends = "last", length = unit(2, "mm"))),
      xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = Inf
    ) +
    labs(title = title)  # add title here
}

p = patchwork::wrap_plots(p1,p2,p3, nrow = 1,tag_level = 'keep',guides = 'keep')

print(p)

ggsave('images_manuscript/supp_traditional_barcodes.pdf', width=8,height = 7)
ggsave('images_manuscript/supp_traditional_barcodes.png', width=8,height = 7,dpi=1200)


```
# Comparison of run times

Now let's compare the time to produce references and to make predictions. Let's start by compiling information to prepare varKoder images.

```{r}
read_csv('Malpighiaceae+Chrysobalanaceae/varKoder/stats.csv') %>%
  select(sample, ends_with('time')) %>%
  group_by(sample) %>%
  summarise_all(sum) %>%
...
```{r}
all_files = list.files('all_SRA/varkoder_images_SRA/',pattern='*.png')
...
n_samples_SRA = data.frame(filename=all_files) %>%
  ...
  mutate(validation_set = ifelse(validation_set, "Validation set", "Training set")) %>%
...
For other datasets, it is similar to SRA.
...
```{r}
N_species = plot_Nsamples_area(n_samples_species, title = expression(italic("Stigmaphyllon") * " species"))
N_genera = plot_Nsamples_area(n_samples_genera,title='Malpighiales senera')
N_families = plot_Nsamples_area(n_samples_SRA,title='Eukaryotic familes') + 
  facet_wrap(~validation_set)

p = patchwork::wrap_plots(N_species,N_genera,N_families, nrow = 1)
