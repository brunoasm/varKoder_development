...
varKoder train --overwrite -b 300 -r 0.1 --single-label -e 30 -z 0 images_cgr_NEW varkoder_trained_model_SL1_images_cgr_FINAL
...
varKoder train -V validation_samples_SL1_images_cgr_FINAL.txt -b 64 -r 0.01 -e 3 -z 5 --pretrained-model varkoder_trained_model_SL1_images_cgr_FINAL/trained_model.pkl images_cgr_NEW varkoder_trained_model_ML1_images_cgr_FINAL


#query
mkdir -p varkoder_query_images_cgr_FINAL

cat validation_samples_SL1_images_cgr_FINAL.txt | tr ',' '\n' | xargs -L 500 -I {} bash -c 'for sample in "$@"; do for file in $(find  images_cgr_NEW/ -name "${sample}@*"); do ln -sf ../"$file" varkoder_query_images_cgr_FINAL/; done; done' bash {}
