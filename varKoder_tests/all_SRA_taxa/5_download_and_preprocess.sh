...
process_record() {
    ...
    if grep -q "^$run\$" "$RUN_ID_FILE"; then
        echo "Skipping $run as images already exist in varKodes/"
        return
    #else
    ...
    local nspots=$(printf "%.0f" $(echo "$ave_spots * 3 + 10000" | bc))
	...
	echo fastq-dump -N $min_spots -X $nspots --skip-technical --gzip --read-filter pass --readids --split-spot --split-files --outdir "$chunk_dir" "$run"
        if timeout 90s fastq-dump -N $min_spots -X $nspots --skip-technical --gzip --read-filter pass --readids --split-spot --split-files --outdir "$chunk_dir" "$run"; then
...
# Read CSV in chunks
chunk_size=500  # Number of SRA records to process in each varKoder run
chunk_number=0
record_count=0
